...
def load_checkpoint(param, use_checkpoint, device):
    """
    loads the most recent (non-corrupted) checkpoint or the final models
    :param param: args number
    :param use_checkpoint: checkpointed or final models
    :return:
    """
    if not use_checkpoint:
        return torch.load(f'./models/diff-params-ARGS={param}/params-final.pt', map_location=device)
    else:
        checkpoints = os.listdir(f'./models/diff-params-ARGS={param}/checkpoint')
        checkpoints.sort(reverse=True)
        for i in checkpoints:
            try:
                file_dir = f"./models/diff-params-ARGS={param}/checkpoint/{i}"
...
def load_parameters(device):
    """
    Loads the trained parameters for the detection models
    :return:
    """
    import sys
