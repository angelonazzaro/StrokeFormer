...
def generate_simplex_noise(
    ...
    for i in range(in_channels):
        ...
        if random_param:
            param = random.choice(
                [(2, 0.6, 16), (6, 0.6, 32), (7, 0.7, 32), (10, 0.8, 64), (5, 0.8, 16), (4, 0.6, 16), (1, 0.6, 64),
                 ...
                 (2, 0.85, 8), (2, 0.85, 4), (2, 0.85, 2), (1, 0.85, 128), (1, 0.85, 64), (1, 0.85, 32),
                 (1, 0.85, 16), (1, 0.85, 8), (1, 0.85, 4), (1, 0.85, 2)]
            )
            noise_field = torch.from_numpy(
        ...
        if noise_field.ndim + 1 == x[:, i].ndim:
            noise_field = noise_field.unsqueeze(0).repeat(batch_size, *([1] * noise_field.ndim))
        elif noise_field.shape != x[:, i].shape:
            noise_field = noise_field.expand(x[:, i].shape)
...
class GaussianDiffusionModel:
    ...
    def predict_eps_from_x_0(self, x_t, t, pred_x_0):
        return (extract(self.sqrt_recip_alphas_cumprod, t, x_t.shape, x_t.device) * x_t - pred_x_0) / extract(
            self.sqrt_recipm1_alphas_cumprod, t, x_t.shape, x_t.device)
    ...
    def p_mean_variance(self, model, x_t, t, estimate_noise=None):
        ...
        # fixed models variance defined as \hat{\beta}_t - could add learned parameter
    ...
    def forward_backward(self, model, x, see_whole_sequence="half", t_distance=None, denoise_fn=None):
    ...
    def p_loss(self, model, x_0, train_start: bool = False, head_masks=None):
        if self.loss_weight is None:
            if train_start:
                t = torch.randint(
                    0, min(self.sample_distance, self.num_timesteps), (x_0.shape[0],),
                    device=x_0.device
                )
            else:
    ...
    def prior_vlb(self, x_0):
        t = torch.tensor([self.num_timesteps - 1] * len(x_0), device=x_0.device)
    ...
    def calc_total_vlb(self, x_0, model):
        ...
        for t in reversed(list(range(self.num_timesteps))):
            t_batch = torch.tensor([t] * len(x_0), device=x_0.device)
