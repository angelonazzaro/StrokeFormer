            ...
            if num_cc_output > 0 and not too_many:  # If there are output instances (and not too many)
                for lbl in torch.unique(cc_output_unique):
                    ...
                    if num_cc_label > 0:
                        ...
                        cc_label_location = torch.zeros(1, device=output_batch.device, dtype=torch.bool,
                                                        requires_grad=False)
                        cc_label_loss = torch.zeros(1, device=output_batch.device, dtype=output_batch.dtype,
                                                    requires_grad=True)
                        detection_map = torch.zeros(1, device=output_batch.device, dtype=torch.bool,
                                                    requires_grad=False)
                    ...
                    if bounding_boxes_output:
                        ...
                        loss_per_output_instance = torch.cat(
                            (loss_per_output_instance, torch.unsqueeze(cc_label_loss, 0)), 0)
                    else:
                ...
                output_intersection = torch.zeros((temp, num_cc_label_temp), device=output_batch.device,
                                                  dtype=torch.bool)
